...
# The following code are part of the code of alchemlyb.
...
class READ_PROD_OUT():
    ...
    def extract_data(self,):
        ...
        #files = glob(self.path_pattern)
        print(files)
        print('Parent process %s.' % os.getpid())
        p = Pool(processes = 12)
        # for file_ in files:
            # p.apply_async(self.read_file, args=(file_,))
        self.u_nk_list = p.map(self.read_file, files)
        print('Waiting for all subprocesses done...')
        p.close()
        p.join()
        # threads = [Thread(target=self.read_file, args=(file_,)) for file_ in files]
        ...
        u_nk_pd = pd.concat(self.u_nk_list,sort=True)
...
#         # print(self.cM2A_path_pattern, self.cM2B_path_pattern)
...
#     all_ts_obj = ANA_ALL_TAIL('F:\\rbfe_test\\rbfe_test\\mcl1\\run-1\\23-27\\FEP')
